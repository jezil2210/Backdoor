...
The idea is connect the both computers(mine and the target machine) using sockets, but trying to connect my computer with the target directly doesn't work, the system denies the connection with some random port obviously, is like someone random trying to connect with my computer i wouldn't allow. So instead of me try to connect with some random port of the target machine i'm gonna make the target machine connect with me, we call this reverse shell, when the target downloads the .exe it's gonna run the progam and this way connecting with some port that i opened in my computer.</br>
